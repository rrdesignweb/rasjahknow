...
const Button = ({ href, className, target = "_self", type, onClick, children, dark, light = true }: ButtonProps) => {
  let defaultClass = `cursor-pointer appearance-none py-2 my-4 px-4 border-2  font-semibold uppercase shadow-md `;
    ...
    <>
      {
        href ? (
          ...
          <button
            ...
            data-button
            className={`cursor-pointer appearance-none py-2 my-4 px-4 bg-transparent border-2  text-white font-semibold shadow-md hover:bg-red-700 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-opacity-75 ${className}`}
          >
            {children}
          </button>
        )
